...
const options = {
  ...
  template: {
    ...
    currentPage:
      '<strong class="tui-page-btn tui-is-selected tui-own-styles">{{page}}</strong>',
    moveButton:
...
function getFirstPage() {
  ...
  if (
    getLocalData(KEY_ADD_BOOKS).length === 0 ||
    getLocalData(KEY_ADD_BOOKS) === null
  ) {
...
pagination.on('afterMove', event => {
  ...
  const sliceBooks = getLocalData(KEY_ADD_BOOKS).slice(
    (pageNumber - 1) * options.itemsPerPage,
